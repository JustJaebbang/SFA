...
def maybe_gradcam_snapshots(
    ...
    # 0) torch.compile 대응: 원본 모듈로 회귀
    base_model = getattr(model, "_orig_mod", model)

    # 1) 타깃 레이어 해석(문자열/모듈/자동) + Conv2d 보장
    def resolve_target_layer(m, name):
        if isinstance(name, str) and name:
            tl = m.get_submodule(name)
        elif isinstance(name, nn.Module):
            tl = name
        else:
            tl = find_target_layer(m)  # 너가 쓰던 자동 탐색 함수

        # tl이 Conv2d 컨테이너/시퀀스면 마지막 Conv2d로 내리깊게 탐색
        if not isinstance(tl, nn.Conv2d):
            last_conv = None
            for n, sub in tl.named_modules() if isinstance(tl, nn.Module) else m.named_modules():
                if isinstance(sub, nn.Conv2d):
                    last_conv = sub
            if last_conv is not None:
                tl = last_conv

        assert isinstance(tl, nn.Module), f"target_layer가 nn.Module이 아닙니다: {type(tl)}"
        assert any(isinstance(x, nn.Conv2d) for x in tl.modules()) or isinstance(tl, nn.Conv2d), \
            f"target_layer가 Conv2d를 포함하지 않습니다: {tl}"
        return tl

    tlayer = resolve_target_layer(base_model, target_layer_name)

    # 2) 대상 레이어 파라미터 grad 보장(부분 freeze 보호)
    for p in tlayer.parameters(recurse=True):
        p.requires_grad_(True)

    # 3) 로짓 수집(계산은 no_grad로, 텐서는 CPU에 보관)
    xb_all, yb_all, logit_all = [], [], []
    seen = 0
    base_model.eval()  # BN/Dropout 고정
    with torch.no_grad():
        for xb, yb in loader:
            if seen >= scan_limit:
                break
            bs = xb.size(0)
            take = min(bs, scan_limit - seen)
            xb = xb[:take].to(device, non_blocking=(device.type == "cuda"))
            yb = yb[:take]
            logits = base_model(xb)
            xb_all.append(xb.detach().cpu())
            yb_all.append(yb.detach().cpu())
            logit_all.append(logits.detach().cpu())
            seen += take

    if seen == 0:
        return 0

    xb_all = torch.cat(xb_all, dim=0)       # [N,C,H,W] on CPU
    yb_all = torch.cat(yb_all, dim=0)       # [N]
    logit_all = torch.cat(logit_all, dim=0) # [N,K]
    probs = torch.softmax(logit_all, dim=1)
    confs, yhat = probs.max(dim=1)

    # 4) 인덱스 선택: 오분류 → 저신뢰 → 정상
    idx_mis = (yhat != yb_all).nonzero(as_tuple=False).flatten().tolist()
    idx_low = (confs < tau).nonzero(as_tuple=False).flatten().tolist()
    idx_ok  = (confs >= tau).nonzero(as_tuple=False).flatten().tolist()

    selected, seen_set = [], set()
    def _take(pool, n):
        cnt = 0
        for i in pool:
            if i not in seen_set:
                selected.append(i); seen_set.add(i); cnt += 1
                if cnt >= n: break
    ...
    # 5) CAM 계산: grad ON, AMP/no_grad OFF, zero_grad → requires_grad on input
    saved = 0
    mean = torch.tensor([0.485, 0.456, 0.406], device=device).view(1,3,1,1)
    std  = torch.tensor([0.229, 0.224, 0.225], device=device).view(1,3,1,1)

    # 진단 로그(필요 시)
    # print(f"[GradCAM] target_layer={tlayer} | is_conv={isinstance(tlayer, nn.Conv2d)}")

    with torch.enable_grad():
        cam = GradCAM(base_model, target_layer=tlayer,
                      use_cuda=(device.type == "cuda"), gradcam_pp=gradcam_pp)
        try:
            for i in selected:
                x1 = xb_all[i:i+1].to(device)          # [1,C,H,W]
                x1.requires_grad_(True)                # 일부 구현에서 필요
                base_model.zero_grad(set_to_none=True) # 이전 그래프 잔여물 제거

                yp = int(yhat[i].item())
                yt = int(yb_all[i].item())
                conf = float(confs[i].item())

                heat, _ = cam(x1, class_idx=yp)        # 여기서 hooks/grad 수집

                # 시각화 원본 복원
                x_vis = (x1 * std + mean).clamp(0,1).squeeze(0).detach().cpu()
                pil = to_pil_image(x_vis)
                over = overlay_cam_on_image(heat, pil)
                over.save(out_dir / f"ep_cam_{saved:03d}_pred-{classes[yp]}_true-{classes[yt]}_conf-{conf:.2f}.png")

                saved += 1
                if saved >= (n_mis + n_low + n_ok):
                    break
        finally:
            # hooks 해제(중복 등록 누적 방지)
            try:
                cam.close()
            except Exception:
                pass
